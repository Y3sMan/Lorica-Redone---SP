import { on, printConsole, Form, Game, Message, Actor, ObjectReference, Spell, Debug, Utility, hooks, once, FormList, Keyword, MagicEffect, PapyrusObject } from  "@skyrim-platform/skyrim-platform";
import { AddMagicEffectToSpell, GetEffectArchetypeAsInt, GetAllSpells, RemoveMagicEffectFromSpell } from  "@skyrim-platform/po3-papyrus-extender/PO3_SKSEFunctions";
import { pl, juKeys, suKeys, FormToString } from "./YM_Lorica_Shared"
import { FormListAdd, FormListCount, Save, FormListGet, FormListRemove, FormListHas } from  "@skyrim-platform/papyrus-util/JsonUtil";
...
export const isRightSpellType = (akForm: Form): boolean => {
	/* 
	SummonCreature = 18
	BoundWeapon    = 17
	Reanimate      = 22
...
export function SetCosts(option: string, akspell?: Form) {
	...
	const main = function ( spell: Form ) {
		...
		if ( spell.hasKeyword(keywordRitual) ) {
			iMag =  fCost * GetFloatValue(null, suKeys.fRitualMult, 0.5);
		}
		else {
...
function AddDescription(spell: Form, iMag: number) {
	...
	for ( var i = 0; i < S.getNumEffects(); i++ ) { if ( S.getNthEffectMagicEffect(i).getFormID() == dummySelf.getFormID() ) { var removeEffect = S.getNthEffectMagicEffect(i); break }};
